...
public class GatewayRoutesConfig {
	...
	public RouteLocator customRouteLocator(
		...
		return builder.routes()
				...
				.route("cms-company-info-company-departments", r -> r
						.path("/api/companies/*/departments", "/api/companies/*/departments/**")
						.filters(f -> f.rewritePath(
								"/api/companies/(?<companyId>[^/]+)/departments(?<segment>/?.*)",
								"/companies/${companyId}/departments${segment}"
						))
						.uri(companyInfoBaseUrl)
				)
				...
				.route("cms-risk-processing-statistic", r -> r
						.path("/api/risks/processing/statistic", "/api/risks/processing/statistic/**")
						.uri(riskBaseUrl)
				)
				...
				.route("cms-workflow-incidents-rule-effectiveness", r -> r
						.path("/api/incidents/rule-effectiveness", "/api/incidents/rule-effectiveness/**")
						.uri(workflowBaseUrl)
				)
				.route("cms-workflow-tasks-my-v1", r -> r
						.path("/api/v1/tasks/my", "/api/v1/tasks/my/**")
						.filters(f -> f.rewritePath("/api/v1/tasks/my(?<segment>/?.*)", "/api/tasks/my${segment}"))
						.uri(workflowBaseUrl)
				)
				.route("cms-workflow-task-by-id-v1", r -> r
						.path("/api/v1/tasks/*", "/api/v1/tasks/*/**")
						.filters(f -> f.rewritePath("/api/v1/tasks/(?<taskId>[^/]+)(?<segment>/?.*)", "/api/tasks/${taskId}${segment}"))
						.uri(workflowBaseUrl)
				)
